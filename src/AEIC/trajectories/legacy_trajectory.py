...
class LegacyTrajectory(Trajectory):
    ...
    def __init__(self, ac_performance:PerformanceModel, mission, optimize_traj:bool,
                 iterate_mass:bool, startMass:float=-1, pctStepClm=0.01,
                 pctStepCrz=0.01,pctStepDes=0.01, fuel_LHV=43.8e6):
        super().__init__(ac_performance, mission, optimize_traj, iterate_mass,
                         startMass=startMass)
        ...
        max_alt = \
            feet_to_meters(ac_performance.model_info['General_Information']['max_alt_ft'])
    ...
    def climb(self):
        ...
        if dAlt < 0:
            raise ValueError("Departure airport + 3000ft should not be higher" \
    ...
    def cruise(self):
        ...
        cruise_dist_approx =\
            nautmiles_to_meters(self.gc_distance)\
                - cruise_start_distance - descent_dist_approx
        ...
        cruise_distance_values =\
            np.linspace(cruise_start_distance, cruise_end_distance, self.NCrz)
    ...
    def descent(self):
        ...
        for field in self.traj_data.dtype.names:
            self.traj_data[field][self.NClm+self.NCrz] =\
                self.traj_data[field][self.NClm+self.NCrz-1]

        dAlt = (self.des_end_altitude - self.des_start_altitude) / (self.NDes)
        if dAlt > 0:
            raise ValueError("Arrival airport + 3000ft should not be higher than"\
                             "end of cruise point")
    ...
    def calc_starting_mass(self):
        ...
        if np.shape(ff_mat) != (2,):
            raise ValueError("Mass estimation fuel flow matrix does not have the"\
                             "required dimensions (Expected: (2,); Recieved: "\
                                f"{np.shape(ff_mat)})")

        # Now perform the necessary interpolations in TAS and fuel flow
        FL_weighting =\
            (self.crz_FL - self.crz_FLs[0]) / (self.crz_FLs[1] - self.crz_FLs[0])
    ...
    def __calc_crz_FLs(self):
        ...
        self.crz_FLs =\
            np.array(self.ac_performance.performance_table_cols[0])[self.crz_FL_inds]
    ...
    def __search_mass_ind(self, mass):
        ...
        '''
        mass_ind_high =\
            np.searchsorted(self.ac_performance.performance_table_cols[-1], mass)
    ...
    def __get_zero_roc_index(self, roc_zero_tol=1e-6):
        ...
        self.zero_roc_mask = np.abs(np.array(
            self.ac_performance.performance_table_cols[2])) < roc_zero_tol
    ...
    def __calc_FL_interp_vals(self, i, alt, roc_perf):
        ...
        non_zero_tas_vals =\
            np.array(self.ac_performance.performance_table_cols[1])[non_zero_tas_inds]

        # ROC will only be valid in descent
        non_zero_roc_inds = non_zero_ff_inds[2]
        non_zero_roc_vals =\
            np.array(self.ac_performance.performance_table_cols[2])[non_zero_roc_inds]
    ...
    def __calc_roc_climb(self, i, FL, seg_start_mass, roc_perf, rocs):
        ...
        bounding_mass =\
            np.array(self.ac_performance.performance_table_cols[3])[mass_inds]
        ...
        mass_weight =\
            (seg_start_mass - bounding_mass[0]) / (bounding_mass[1] - bounding_mass[0])
    ...
    def __legacy_climb(self):
        ...
        pos_roc_perf = self.ac_performance.performance_table[
            np.ix_( np.arange(self.ac_performance.performance_table.shape[0]),
                   # axis 0: flight levels
                    np.arange(self.ac_performance.performance_table.shape[1]),
                    # axis 1: all TAS's
        ...
        for i in range(0, self.NClm-1):
            ...
            segment_time =\
                (self.traj_data['altitude'][i+1] - self.traj_data['altitude'][i]) / roc
            ...
            self.traj_data['acMass'][i+1] =\
                self.traj_data['acMass'][i] - segment_fuel
            self.traj_data['groundDist'][i+1] =\
                self.traj_data['groundDist'][i] + dist
    ...
    def __legacy_descent(self):
        ...
        for i in range(startN, endN):
            alt = self.traj_data['altitude'][i]
            tas_interp, ff_interp, roc_interp =\
                self.__calc_FL_interp_vals(i, alt, neg_roc_perf)
